...
class WiktionaryEtymologyService {
  ...
  String _getSubdomain(String languageCode) {
    return _languageSubdomains[languageCode] ??
        'en'; // Default to English if language not supported
  }

  /// Fetch detailed Wiktionary information for a given word in a specific language
  Future<List<WiktionaryEntry>> fetchWordDetails(
    String word,
    String language,
  ) async {
    try {
      ...
      final response = await http
          .get(
            Uri.parse(
              '$baseUrl?action=parse&page=$word&prop=wikitext&format=json',
            ),
  ...
  List<Map<String, dynamic>> _extractDefinitions(String content) {
    ...
    for (final line in lines) {
      ...
      // Look for antonyms starting with #*:
        ...
        List<String> antonyms = antonymsText
            .split(',')
            .map((s) => _cleanWikitext(s.trim()))
            .toList();
  ...
  Future<EtymologyResult> fetchEtymology(String word, String language) async {
    try {
      ...
      final sections = entries
          .map(
            (entry) => EtymologySection(
              title: 'Etymology: ${entry.partOfSpeech}',
              language: language,
              originalLanguage: language,
              content: entry.etymology.isNotEmpty
                  ? entry.etymology
  ...
  Future<EtymologyResult> fetchEtymologyDetailed(
    String word,
    String language,
  ) async {
