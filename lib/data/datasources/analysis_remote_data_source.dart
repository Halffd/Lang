...
class AnalysisRemoteDataSource {
  ...
  static const List<String> _allowedHosts = ['localhost', '127.0.0.1', '[::1]'];

  final String _apiUrl;

  AnalysisRemoteDataSource({String? apiUrl})
    : _apiUrl = _validateAndSanitizeUrl(apiUrl ?? _defaultApiUrl);
  ...
  static String _validateAndSanitizeUrl(String url) {
    try {
      ...
      if (uri.scheme != 'http' && uri.scheme != 'https') {
        throw ArgumentError(
          'Invalid scheme: ${uri.scheme}. Only http/https allowed.',
        );
      }
  ...
  /// Fallback chunker for Chinese when the remote tokenizer is
  /// unavailable: takes the longest run that fits (4..2 chars),
  /// then single characters. No dictionary involved.
  List<String> _simpleChineseTokenize(String text) {
    final List<String> tokens = [];
    int i = 0;
    while (i < text.length) {
      final maxLen = 4 < text.length - i ? 4 : text.length - i;
      final len = maxLen >= 2 ? maxLen : 1;
