  ...
  void _confirmDeleteDeck(SrsDeck deck) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Delete "${deck.name}"? ${cardCount > 0 ? '$cardCount cards will be moved to No Deck.' : ''}',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            ...
            child: decks.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (ctx, idx) {
                      ...
                      final cardCount = widget.srsService.getDeckCardCount(
                        deck.id,
                      );
                      final dueCount = widget.srsService.getDeckDueCount(
                        deck.id,
                      );
                      ...
                      return ListTile(
                        ...
                        subtitle: Text(
                          '${deck.description ?? ''} • $cardCount cards${dueCount > 0 ? ' • $dueCount due' : ''}',
                        ),
                        trailing: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
