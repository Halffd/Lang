...
class AddCardSheet extends StatefulWidget {
  ...
  const AddCardSheet({this.editCard, required this.onSave, required this.srsService, super.key});
...
class _AddCardSheetState extends State<AddCardSheet> {
          ...
          body: ListView(
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                ...
                items: widget.srsService.decks.map((d) => DropdownMenuItem(value: d.id, child: Text(d.name))).toList(),
                ...
                children: [
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text('Priority: $_priority', style: const TextStyle(fontWeight: FontWeight.bold)),
                        Slider(
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Language Level: $_languageLevel', style: const TextStyle(fontWeight: FontWeight.bold)),
                        Slider(
              ...
              Wrap(
                spacing: 8,
                children: [
                  OutlinedButton.icon(onPressed: _pickImage, icon: const Icon(Icons.photo), label: const Text('Image')),
                  OutlinedButton.icon(onPressed: _takeScreenshot, icon: const Icon(Icons.screenshot_monitor), label: const Text('Screenshot')),
                  OutlinedButton.icon(onPressed: _pickVideo, icon: const Icon(Icons.videocam), label: const Text('Video')),
                ],
              ),
  ...
  void _saveCard() async {
    ...
    try {
      final card = SRSCard(
        id: widget.editCard?.id ?? '${_wordController.text.trim()}_${DateTime.now().millisecondsSinceEpoch}',
        word: _wordController.text.trim(),
        reading: _readingController.text.trim().isEmpty ? null : _readingController.text.trim(),
        ...
        createdAt: widget.editCard?.createdAt ?? DateTime.now(),
