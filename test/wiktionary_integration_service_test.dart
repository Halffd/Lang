...
void main() {
  group('WiktionaryIntegrationService', () {
    ...
    test(
      'should enrich search result with Wiktionary details when details exist',
      () async {
        // Arrange
        ...
        // Act
        final result = await wiktionaryService.enrichSearchResult(
          baseResult,
          query,
          detectedLanguage,
        );
        ...
        expect(
          result.wiktionaryDetails['${query}_']![0].language,
          detectedLanguage,
        );
        expect(
          result.wiktionaryDetails['${query}_']![0].definition,
          'definition 1',
        );
        expect(
          result.wiktionaryDetails['${query}_']![1].definition,
          'definition 2',
        );
      },
    );
    ...
    test(
      'should return base result unchanged when no Wiktionary details exist',
      () async {
        // Arrange
        final baseResult = SearchResult(
          entries: [MockDictionaryEntry()],
    ...
    test('should handle exceptions gracefully', () async {
      ...
      expect(
        result,
        equals(baseResult),
      ); // Should return base result when exception occurs
    });
  });
}
